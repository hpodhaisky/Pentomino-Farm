...
function order(n::Int64, z, width, height, pieces)  # determine order of pieces
    ...
    for x in 1 : width
        for y in 1 : height
            if (x + y) % 2 == 1
                for d in ((0, -1), (1, 0), (2, 1), (0, 1), (-2, 1), (-1, 0))
                    neigh = (x, y) .+ d
                ...
                for d in ((0, -1), (2, -1), (1, 0), (0, 1), (-1, 0), (-2, -1))
                    neigh = (x, y) .+ d
...
function solvePolyiamond(area = Pair(24, 24))
    ...
    for p in 1 : length(pieces)  # orientations which were left out because of symmetries are disallowed
    ...
    @showprogress "Saving solutions to disk." for i in 2 : result_count(model)
        try
    ...
    writedlm("solutions-tri.csv",  output, ',')
